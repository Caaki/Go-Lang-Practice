...
Use an infinite for loop and a select statement to log the
emails and sms messages as they come in order across the two channels.
Add a condition to return from the function when one of the two channels closes,
whichever is first.
...
func logMessages(chEmails, chSms chan string) {
	...
	for {
		select {
		case eMessage, ok := <-chEmails:
			if !ok {
				return
			}
			logEmail(eMessage)
		case smsMessage, ok := <-chSms:
			if !ok {
				return
			}
			logSms(smsMessage)
