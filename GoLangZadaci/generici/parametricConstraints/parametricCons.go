...
Specifically, she wants us to create a new biller interface.
...
A customer is either a user or an org. A user will be billed with a userBiller and an org with an
orgBiller.
...
Charge
Name
The good news is that the architect already wrote the userBiller and orgBiller types for us that
fulfill this new biller interface.
Use the definitions of those types and their methods to figure out how to write the biller interface
definition starting on line 7.
*/
import (
	"fmt"
)

type biller[T customer] interface {
	Charge(T) bill
	Name() string
}
